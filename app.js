const STORAGE_KEY = "salon_app_v42";
...
const longMonthNames = ["januari","februari","maart","april","mei","juni","juli","augustus","september","oktober","november","december"];

function seedData() {
  if (localStorage.getItem(STORAGE_KEY)) return;
  const data = {
    customers: [
      { id: 1, name: "Kim van Dijk", phone: "06 12345678", email: "kim@example.com", note: "Komt liefst in de voormiddag." },
      { id: 2, name: "Lotte van Dam", phone: "06 87654321", email: "lotte@example.com", note: "Nieuwe klant sinds februari." },
      { id: 3, name: "Loes Maas", phone: "06 11223344", email: "loes@example.com", note: "Verkiest luxe behandeling." },
      { id: 4, name: "Lisa Jansen", phone: "06 99887766", email: "lisa@example.com", note: "Wil graag een sms-herinnering." }
    ],
    services: [
    ...
    appointments: [
      { id: 1, customerId: 2, serviceId: 1, date: "2024-03-20", time: "10:00", duration: 60, price: 65, status: "gepland", paid: false, paymentMethod: "" },
      { id: 2, customerId: 3, serviceId: 2, date: "2024-03-20", time: "11:30", duration: 90, price: 95, status: "gepland", paid: false, paymentMethod: "" },
      { id: 3, customerId: 1, serviceId: 1, date: "2024-03-20", time: "14:00", duration: 60, price: 65, status: "gepland", paid: true, paymentMethod: "cash" },
      { id: 4, customerId: 3, serviceId: 2, date: "2024-03-21", time: "11:30", duration: 90, price: 95, status: "afgerond", paid: true, paymentMethod: "bancontact" },
      { id: 5, customerId: 1, serviceId: 1, date: "2024-03-21", time: "14:00", duration: 60, price: 65, status: "gepland", paid: false, paymentMethod: "" },
      { id: 6, customerId: 4, serviceId: 3, date: "2024-03-21", time: "16:00", duration: 75, price: 85, status: "no-show", paid: false, paymentMethod: "" }
    ]
  };
...
function euro(value){ return new Intl.NumberFormat("nl-BE",{style:"currency",currency:"EUR"}).format(Number(value||0)); }
function formatLongDate(dateStr){ const d=new Date(dateStr+"T00:00:00"); return `${d.getDate()} ${longMonthNames[d.getMonth()]} ${d.getFullYear()}`; }
function formatShortDate(dateStr){ const d=new Date(dateStr+"T00:00:00"); return `${d.getDate()} ${longMonthNames[d.getMonth()]}`; }
function nextId(items){ return items.length ? Math.max(...items.map(i=>i.id))+1 : 1; }
function customerById(data,id){ return data.customers.find(c=>c.id===id); }
function serviceById(data,id){ return data.services.find(s=>s.id===id); }

function updateTopbar(screenId, title){
  document.getElementById("screenTitle").textContent = title;
  const backBtn = document.getElementById("backBtn");
  const topAdd = document.getElementById("fabAddTop");
  const fab = document.getElementById("floatingAddBtn");

  backBtn.classList.toggle("hidden-btn", screenId !== "clientDetailScreen");

  if (screenId === "agendaScreen") {
    topAdd.onclick = openNewAppointmentDialog;
    fab.onclick = openNewAppointmentDialog;
    topAdd.style.visibility = "visible";
    fab.style.display = "block";
  } else if (screenId === "clientsScreen") {
    topAdd.onclick = openNewClientDialog;
    fab.onclick = openNewClientDialog;
    topAdd.style.visibility = "visible";
    fab.style.display = "block";
  } else if (screenId === "servicesScreen") {
    topAdd.onclick = openNewServiceDialog;
    fab.onclick = openNewServiceDialog;
    topAdd.style.visibility = "visible";
    fab.style.display = "block";
  } else if (screenId === "revenueScreen") {
    topAdd.style.visibility = "hidden";
...
function renderAgendaList() {
  ...
  const appts = data.appointments.filter(a => a.date === state.selectedDate).sort((a,b)=>a.time.localeCompare(b.time));
  ...
  appts.forEach(app => {
    ...
    row.innerHTML = `
      ...
      <div>
        <div class="main-name">${customer ? customer.name : "Onbekend"}</div>
  ...
  list.querySelectorAll(".price-chip").forEach(btn => btn.addEventListener("click", (e) => {
    e.stopPropagation();
...
function renderClients() {
  ...
  const clients = data.customers.filter(c => [c.name,c.phone,c.email].join(" ").toLowerCase().includes(q));
  if (!clients.length) {
  ...
  clients.forEach(client => {
    ...
    card.innerHTML = `<button type="button" data-id="${client.id}"><div class="client-name">${client.name}</div><div class="meta">${client.phone || "Geen telefoon"} · ${count} afspraken</div></button>`;
...
function renderServices() {
  ...
  data.services.forEach(service => {
    ...
    card.innerHTML = `<button type="button" data-id="${service.id}"><div class="client-name">${service.name}</div><div class="meta">${service.duration} min · ${euro(service.price)}</div></button>`;
...
function openClientDetail(clientId) {
  ...
  const appts = data.appointments.filter(a => a.customerId === clientId).sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`));
  content.innerHTML = `
    <div class="detail-card">
      <div class="detail-block">
        <div class="inline-header">
          ...
          <button class="ghost-btn" id="editClientBtn" type="button">Bewerk klant</button>
        </div>
      </div>
      <div class="detail-block">
        <div class="info-grid">
          <div><div class="detail-label">Telefoon</div><div class="detail-value">${client.phone || "-"}</div></div>
          <div><div class="detail-label">E-mail</div><div class="detail-value">${client.email || "-"}</div></div>
        </div>
      </div>
      <div class="detail-block">
    ...
    <div class="detail-card">
      <div class="detail-block">
        <div class="inline-header">
          ...
          <button class="ghost-btn" id="newClientAppointmentBtn" type="button">Nieuwe afspraak</button>
        </div>
      </div>
      ${appts.length ? appts.map(app => {
        const service = serviceById(data, app.serviceId);
        return `<div class="detail-block"><div class="inline-header"><div><div class="detail-value">${formatShortDate(app.date)} · ${app.time}</div><div class="meta">${service ? service.name : ""} · ${app.status} · ${euro(app.price)}${app.paid ? " · " + app.paymentMethod : ""}</div></div><button class="ghost-btn from-detail-edit" data-id="${app.id}" type="button">Bewerk</button></div></div>`;
      }).join("") : `<div class="detail-block"><div class="detail-value">Nog geen afspraken.</div></div>`}
    </div>
  `;
  ...
  content.querySelectorAll(".from-detail-edit").forEach(btn => btn.addEventListener("click", ()=>openEditAppointmentDialog(Number(btn.dataset.id))));
...
function renderRevenue() {
  ...
  const monthAppts = data.appointments.filter(a => a.date.startsWith(monthPrefix));
  const paid = monthAppts.filter(a => a.paid).reduce((sum,a)=>sum+Number(a.price||0),0);
  const planned = monthAppts.filter(a => a.status !== "no-show").reduce((sum,a)=>sum+Number(a.price||0),0);
  const open = monthAppts.filter(a => !a.paid && a.status !== "no-show").reduce((sum,a)=>sum+Number(a.price||0),0);
  ...
  const byMethod = {};
  monthAppts.filter(a => a.paid).forEach(a => {
  ...
  if (!methods.length) {
    list.innerHTML = `<div class="empty-state">Nog geen betalingen in deze maand.</div>`;
    return;
  }
  list.innerHTML = "";
  methods.forEach(([method, amount]) => {
    ...
    card.innerHTML = `<div class="detail-block"><div class="inline-header"><div class="detail-value">${method}</div><div class="detail-value">${euro(amount)}</div></div></div>`;
    list.appendChild(card);
  });
}
...
function populateAppointmentForm(customerId=null) {
  ...
  customerSelect.innerHTML = data.customers.map(c => `<option value="${c.id}">${c.name}</option>`).join("");
...
function openNewAppointmentDialog(prefillCustomerId=null) {
  ...
  const serviceSelect = document.getElementById("appointmentService");
  if (serviceSelect.options.length) serviceSelect.value = serviceSelect.options[0].value;
  syncServiceDefaults();
...
function saveAppointmentFromForm(event) {
  ...
  if (id) {
    ...
    data.appointments.push({ id: nextId(data.appointments), ...payload, paid: false, paymentMethod: "" });
  }
  saveData(data);
...
function saveClientFromForm(event) {
  ...
  renderClients();
  if (state.selectedClientId) openClientDetail(state.selectedClientId);
}
...
function registerEvents() {
  document.getElementById("prevMonthBtn").addEventListener("click", () => {
    ...
    renderCalendar(); renderAgendaList(); renderRevenue();
  });
  ...
  document.querySelectorAll(".nav-btn").forEach(btn => btn.addEventListener("click", () => switchScreen(btn.dataset.screen, btn.dataset.title)));
  document.getElementById("backBtn").addEventListener("click", () => {
    const map = { agendaScreen: "Agenda", clientsScreen: "Klanten", servicesScreen: "Diensten", revenueScreen: "Omzet" };
